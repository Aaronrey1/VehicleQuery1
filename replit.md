...
- **Two-Column Layout Refactor:** AI Search, Bulk Search, and VIN Decoder now use a responsive two-column layout. On desktop: results appear on the LEFT (lg:flex-1), form/inputs on the RIGHT (lg:w-80). On mobile: form appears first, results below. This improves usability by keeping the form visible while viewing results.
- **Compact Input Styling:** All search inputs reduced to height h-8 with smaller text (text-sm/text-xs) for a more compact, professional appearance.
...
- **Confidence Scoring Simplified:** Removed ±10 year database tier. Updated AI Search confidence levels: 100% (Exact Match), 80-95% (DB ±5yr), 60-100% (Gemini AI - often 90%+ high accuracy predictions).
