...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // PDF Guide Endpoint
  app.get("/api/guides/search-modes.pdf", (req, res) => {
    try {
      const PDFDocument = require("pdfkit");
      const doc = new PDFDocument();

      res.setHeader("Content-Type", "application/pdf");
      res.setHeader("Content-Disposition", 'attachment; filename="VehicleDB-Search-Modes-Guide.pdf"');

      doc.pipe(res);

      // Title
      doc.fontSize(24).font("Helvetica-Bold").text("VehicleDB Pro", 50, 50);
      doc.fontSize(18).font("Helvetica-Bold").text("Search Modes Guide", 50, 80);
      doc.fontSize(10).font("Helvetica").text("Complete guide to all search modes and how to use them", 50, 105);
      doc.moveTo(50, 120).lineTo(550, 120).stroke();

      let y = 140;

      // AI Search Section
      doc.fontSize(16).font("Helvetica-Bold").text("1. AI-Powered Smart Search", 50, y);
      y += 30;
      doc.fontSize(11).font("Helvetica-Bold").text("How It Works:", 50, y);
      y += 15;
      doc.fontSize(10).font("Helvetica").text(
        "AI Search uses a 3-tier intelligent prediction system to find compatibility data:\n\n" +
        "• Tier 1 (100% Exact Match): Direct database lookup - completely reliable\n" +
        "• Tier 2 (80-95% High): Database pattern matching within ±5 years - highly reliable\n" +
        "• Tier 3 (60-79% High): AI-powered prediction via Gemini - very accurate predictions\n",
        50, y, { width: 500, align: "left" }
      );
      y += 100;

      doc.fontSize(11).font("Helvetica-Bold").text("How to Use:", 50, y);
      y += 15;
      doc.fontSize(10).font("Helvetica").text(
        "1. Enter the vehicle Make (e.g., Tesla)\n" +
        "2. Enter the vehicle Model (e.g., Model 3)\n" +
        "3. Enter the vehicle Year (e.g., 2023)\n" +
        "4. Click 'Search' button\n" +
        "5. Results will show predicted Port Type and Device Type with confidence score\n" +
        "6. Optionally, enter your name and email to receive an email when admin approves the prediction\n",
        50, y, { width: 500, align: "left" }
      );
      y += 120;

      doc.fontSize(11).font("Helvetica-Bold").text("Confidence Levels:", 50, y);
      y += 15;
      doc.fontSize(10).font("Helvetica").text(
        "• 100%: Exact database match - use with complete confidence\n" +
        "• 80-95%: Database pattern match (±5 years) - very reliable, minimal review needed\n" +
        "• 60-79%: Gemini AI prediction - accurate but review before using\n",
        50, y, { width: 500, align: "left" }
      );
      y += 80;

      // Bulk Search Section
      doc.fontSize(16).font("Helvetica-Bold").text("2. Bulk Search", 50, y);
      y += 30;
      doc.fontSize(11).font("Helvetica-Bold").text("How It Works:", 50, y);
      y += 15;
      doc.fontSize(10).font("Helvetica").text(
        "Bulk Search allows you to search for multiple vehicles at once using a CSV file. " +
        "This is ideal for processing large lists of vehicles efficiently.",
        50, y, { width: 500, align: "left" }
      );
      y += 50;
      ...
      y += 15;
      doc.fontSize(10).font("Helvetica").text(
        "1. Prepare a CSV file with columns: make, model, year (required columns)\n" +
        "2. Click 'Choose File' to select your CSV\n" +
        "3. Click 'Search' button\n" +
        "4. System processes all vehicles and displays results in a table\n" +
        "5. Click 'Export Results' to download findings as CSV\n\n" +
        "CSV Format Example:\nmake,model,year\nTesla,Model 3,2023\nBMW,X5,2022\n",
        50, y, { width: 500, align: "left" }
      );
      y += 120;
      ...
      // VIN Decoder Section
      doc.fontSize(16).font("Helvetica-Bold").text("3. VIN Decoder", 50, y);
      y += 30;
      doc.fontSize(11).font("Helvetica-Bold").text("How It Works:", 50, y);
      y += 15;
      doc.fontSize(10).font("Helvetica").text(
        "VIN Decoder extracts vehicle information from Vehicle Identification Numbers using " +
        "the NHTSA database, then uses AI to predict compatibility data.",
        50, y, { width: 500, align: "left" }
      );
      y += 60;
      ...
      doc.fontSize(10).font("Helvetica").text(
        "Single VIN Decode:\n" +
        "1. Paste a valid 17-character VIN\n" +
        "2. Click 'Decode' button\n" +
        "3. Results show extracted vehicle data and predicted compatibility\n\n" +
        "Bulk VIN Decode:\n" +
        "1. Prepare a CSV file with a 'vin' column\n" +
        "2. Upload the file\n" +
        "3. System decodes all VINs and returns results with extracted data\n",
        50, y, { width: 500, align: "left" }
      );
      y += 130;

      // Geometris Section
      doc.fontSize(16).font("Helvetica-Bold").text("4. Geometris - Harness Search", 50, y);
      y += 30;
      doc.fontSize(11).font("Helvetica-Bold").text("How It Works:", 50, y);
      y += 15;
      doc.fontSize(10).font("Helvetica").text(
        "Geometris is a specialized search system for vehicle harness compatibility. " +
        "It searches by make and model to find compatible harness types.",
        50, y, { width: 500, align: "left" }
      );
      y += 60;
      ...
      doc.fontSize(10).font("Helvetica").text(
        "1. Enter the vehicle Make (e.g., Tesla)\n" +
        "2. Enter the vehicle Model (e.g., Model S)\n" +
        "3. Click 'Search' button\n" +
        "4. Results display compatible harness types with applicable year ranges\n" +
        "5. Each harness shows the year range it's compatible with\n",
        50, y, { width: 500, align: "left" }
      );
      y += 100;
      ...
      y += 20;
      doc.fontSize(14).font("Helvetica-Bold").text("Billing Information", 50, y);
      y += 25;
      doc.fontSize(10).font("Helvetica").text(
        "Free Searches:\n" +
        "• Exact database matches (100% confidence)\n" +
        "• Database pattern matching within ±5 years\n" +
        "• All searches up to tier 2\n\n" +
        "Paid Searches:\n" +
        "• Gemini AI predictions: $0.01 per request\n" +
        "• Only used when no database matches are found\n",
        50, y, { width: 500, align: "left" }
      );
